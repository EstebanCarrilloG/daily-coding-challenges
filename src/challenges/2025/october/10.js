...
For the final day of Space Week, you will be given the mass in kilograms (kg) of a payload 
you want to send to orbit. Determine the amount of fuel needed to send your payload to orbit 
using the following rules:
    ...
    * To calculate the total fuel needed: start with the payload mass, 
      ...
      and calculate again. Repeat this process until the additional fuel required 
      is less than 1 kg, then stop.
...
lift - 0.4 additional kg - which is less 1 additional kg, so we stop here. 
The total mass to lift is 62.4 kg, 50 of which is the initial payload and 12.4 of fuel.
...
/**
 * Calculate the amount of fuel needed to send a payload to orbit.
 * @param {number} payload - The mass in kilograms (kg) of the payload.
 * @returns {number} The amount of fuel needed rounded to one decimal place.
 * @example
 * launchFuel(50) // returns 12.4
 */
export function launchFuel(payload) {
  let fuelNeeded = 0;
  ...
  return parseFloat(fuelNeeded.toFixed(1));
}
